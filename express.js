...
passport.deserializeUser(function (username, done) {
    ...
    done(null, authData2[username]);
});
passport.use(new LocalStrategy(
    function (username, password, done) {
        console.log('LocalStrategy', username, password, authData2[username]);
        if(authData2[username]){
            if(authData2[username].password === password){
...
server.get('/', function (request, response) {
    ...
    var content = templateHTML(listTag, title, desc, displayName);
